...
public interface PostRepository extends JpaRepository<PostEntity, Long> {
    ...
    @Query("SELECT p.id FROM PostEntity p WHERE p.region = :region AND p.date >=  CURRENT_TIMESTAMP AND p.userEntity.id NOT IN (:blockerIds)")
    List<Long> findIdsByRegionExcludingBlockedUsersAndExpiredDate(Region region, List<Long> blockerIds);
